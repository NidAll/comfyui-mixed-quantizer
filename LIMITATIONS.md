...
7. **Header/metadata size**: per-layer quantization config is stored compactly in
   `_quantization_metadata`; per-tensor metrics and exclusion reasons live in the
   JSON report, not in safetensors metadata.
8. **No network access** during conversion; nothing is downloaded automatically.
9. **Reference drift**: comfy-kitchen/ComfyUI may change the format in the future;
